...
#define ALGORITMOGAUSSSIMPLES_H

#include <vector>
#include <iostream>

class AlgoritmoGaussSimples
{
    public:
        AlgoritmoGaussSimples(const std::vector<std::vector<double>> &matriz)
            : _matriz {matriz}, _respostas(matriz.size(), 0),
              _inferior {false}, _superior {false}, _dividiu {false}
        {}

        AlgoritmoGaussSimples(std::vector<std::vector<double>> &&matriz)
            : _matriz {matriz}, _respostas(matriz.size(), 0),
              _inferior {false}, _superior {false}, _dividiu {false}
        {}
