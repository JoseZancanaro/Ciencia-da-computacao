...
-- Respostas
-- 1
SELECT count(id) AS qtd_projeto FROM projeto;

-- 2
SELECT count(id) AS projeto_concluido FROM projeto WHERE concluido = true;

-- 3
SELECT	count(id) AS qtd_tarefa_finalizada, sum(tempo_reportado) AS tempo_total 
FROM tarefa WHERE concluida = true;
...
-- 5
SELECT max(tempo_reportado) AS tarefa_realizada_mais_demorada FROM tarefa 
WHERE concluida = true;

-- 6
SELECT avg(tempo_estimado) AS media_tempo_estimado FROM tarefa;

-- 7
SELECT projeto.nome AS nome_projeto, tarefa.descricao as tarefa_relacionada FROM projeto 
...
-- 13
SELECT projeto.nome AS nome_projeto, count(tarefa.projeto_id) AS qtd_tarefas_por_projetos FROM projeto
LEFT OUTER JOIN tarefa ON (projeto.id = tarefa.projeto_id)
GROUP BY projeto.nome;

-- 14
SELECT cargo.nome AS nome_cargo, count(colaborador.cargo_id) AS qtd_colaborador_por_cargo FROM cargo
RIGHT OUTER JOIN colaborador ON (cargo.id = colaborador.cargo_id)
GROUP BY cargo.nome;

-- 15 
SELECT projeto.nome AS nome_projeto, sum(tarefa.tempo_reportado) AS tempo_gasto_pro_projeto FROM projeto
LEFT JOIN tarefa ON (projeto.id = tarefa.projeto_id)
GROUP BY projeto.nome;

-- 16
SELECT colaborador.nome AS nome_colaborador, count(tarefa.concluida) AS qtd_tarefas_concluidas FROM colaborador
...
-- 17
SELECT colaborador.nome AS nome_colaborador, count(tarefa.concluida) AS qtd_tarefas_necessarias FROM colaborador
LEFT outer JOIN tarefa ON (colaborador.id = tarefa.colaborador_id)
...
-- 18
SELECT colaborador.nome AS nome_colaborador, sum(tarefa.tempo_reportado) AS tempo_trabalhado FROM colaborador
INNER JOIN tarefa ON (colaborador.id = tarefa.colaborador_id)
GROUP BY colaborador.nome
ORDER BY colaborador.nome;

-- 19
SELECT projeto.nome AS nome_projeto, sum(tarefa.tempo_estimado) AS tempo_estimado_de_conclusao FROM projeto
LEFT OUTER JOIN tarefa ON (projeto.id = tarefa.projeto_id)
GROUP BY projeto.nome;
